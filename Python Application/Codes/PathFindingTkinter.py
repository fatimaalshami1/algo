...
def StartAlg(Algorithm):
    ...
    UI_frame = Frame(root, width=600, height=500, bg='cyan')
    ...
    def StartAlgorithm():
        ...
        if algMenu.get() == 'A-star Algorithm':
            ...
            depth_first(grid_array,speedScale.get())
        #elif algMenu.get() == 'Best-First Algorithm':
            #best_first(grid_array,speedScale.get())
    ...
    algMenu = ttk.Combobox(UI_frame, textvariable=selected_alg, 
                        values=['A-star Algorithm', 'Breadth-First Algorithm','Depth-First Algorithm'], font = font)
    algMenu.current(2)
    #algMenu.set(Algorithm)
    algMenu.grid(row=3, column=0, padx=5, pady=(20, 5), sticky=W)
    algMenu.current(0)
    speedScale = Scale(UI_frame, from_=10, to=1000, #digits=3, resolution=0.005, 
